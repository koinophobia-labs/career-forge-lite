...
function cheapestTierWith(feature: EntitledFeature) {
  return PACKAGE_ORDER.map((tier) => PACKAGES[tier]).find((pack) => pack.features.includes(feature)) ?? PACKAGES.career;
}
  ...
  return (
    <div className="rounded-xl border border-gold/30 bg-gold/5 p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-paper/70">
        Everything you have built here stays yours and stays free to edit — the {pack.name} (${pack.priceUsd}) unlocks
        this on devices where you redeem its access code.
      </p>
...
export function LockedActionPill({ feature, label }: { feature: EntitledFeature; label: string }) {
  ...
  return (
    <Link
      ...
      title={`${label} is included in the ${pack.name} ($${pack.priceUsd}).`}
    >
