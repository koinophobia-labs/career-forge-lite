...
const statusStyles: Record<ApplicationStatus, string> = {
  drafting: "border-white/25 text-paper/60", applied: "border-cyan/50 bg-cyan/10 text-cyan",
  interviewing: "border-gold/50 bg-gold/10 text-gold", offer: "border-spruce/60 bg-mint/10 text-mint",
  rejected: "border-coral/50 bg-coral/10 text-coral", closed: "border-white/20 text-paper/40"
};
...
export function ApplicationsWorkspace() {
  ...
  function addApplication() {
    ...
    update((current) => ({
      ...
      applications: [...current.applications, {
        id: createId("app"), company: company.trim() || "Unknown company", roleTitle: roleTitle.trim() || "Untitled role",
  ...
  function patchApplication(id: string, patch: Partial<ApplicationRecord>) {
    ...
    update((current) => ({
      ...current,
      applications: current.applications.map((application) => application.id === id ? { ...application, ...patch, updatedAt } : application)
    }));
  }
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
        ...
        {hydrated && !sorted.length && <div className="mt-8 rounded-xl border border-cyan/25 bg-cyan/10 p-5 text-sm leading-6 text-paper/72">No applications tracked yet. The fastest path is to <Link href="/tailor" className="font-bold text-cyan underline-offset-2 hover:underline">paste a real job posting</Link>, analyze it, and save the workspace.</div>}
        ...
        {hydrated && sorted.length > 0 && (
          <div className="mt-8 grid gap-3">
            {sorted.map((application) => {
              ...
              const timing = application.status === "interviewing" ? interviewTiming(application, nowIso) : null;
              const pastInterview = timing === "past";
              const unscheduledInterview = timing === "unscheduled";
              const stageHistory = application.stageHistory ?? [];
              const interviewHistory = application.interviewHistory ?? [];
              return (
                <article id={`application-${application.id}`} key={application.id} className={`trust-panel scroll-mt-24 p-4 sm:p-5 ${followUpDue ? "border-gold/40" : application.status === "offer" ? "border-mint/45" : pastInterview || unscheduledInterview ? "border-cyan/40" : ""}`}>
                  <div className="flex flex-wrap items-start gap-3">
                    <div className="mr-auto min-w-0">
                      ...
                      <div className="mt-3 flex flex-wrap gap-3 text-xs">
                        ...
                        {application.status === "interviewing" && (timing === "today" || timing === "upcoming") && <Link href={`/interview?applicationId=${application.id}`} className="rounded-md border border-cyan/35 px-3 py-2 font-bold text-cyan">Practice this interview</Link>}
                      ...
                      {linkedSprints.length > 0 && <p className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[0.72rem] leading-4"><span className="uppercase text-paper/45">Role Sprints:</span>{linkedSprints.map((sprint) => <Link key={sprint.id} href={`/role-sprint?id=${sprint.id}`} className="text-cyan/85 underline-offset-2 hover:text-gold hover:underline">{sprint.title || sprint.requirement} · {sprintTrackerLabel(state, sprint)}</Link>)}</p>}
                    </div>
