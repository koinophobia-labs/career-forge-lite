...
const secondaryOutputFields: Array<{ key: keyof Omit<RoleSprintOutputs, "userEdited" | "resumeBullet">; label: string; hint: string; rows: number }> = [
  { key: "portfolioTitle", label: "Portfolio title", hint: "A heading for this practice artifact.", rows: 2 },
  { key: "portfolioSummary", label: "Portfolio summary", hint: "A short explanation of what you practiced and produced.", rows: 4 },
    ...
    <section className="mx-auto max-w-4xl px-5 py-10 sm:px-8">
            ...
            <ol className="mt-4 grid gap-2 text-sm leading-6 text-paper/75">
              {sprint.instructions.map((step, index) => (
                <li key={step} className="rounded-lg border border-white/10 bg-obsidian/40 p-3">
                  <span className="lab-mono mr-2 text-xs font-bold text-cyan">{index + 1}.</span>{step}
                </li>
              ))}
            </ol>
            <p className="mt-5 text-xs font-black uppercase tracking-wide text-paper/50">Done means</p>
            <ul className="mt-2 grid gap-1.5 text-sm leading-6 text-paper/70">
              {sprint.completionCriteria.map((criterion) => <li key={criterion}>✓ {criterion}</li>)}
            </ul>
          </div>
      ...
      <div className="trust-panel mt-6 p-5 sm:p-6">
        ...
        <textarea
          ...
          rows={12}
          placeholder="Write or paste your completed task here…"
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          {evidenceState !== "approved" && (
            <button type="button" onClick={() => submitWork(sprint)} disabled={workLength < ROLE_SPRINT_MIN_WORK_CHARS} className="lab-pill-button ml-auto px-5 py-2.5 text-sm font-black disabled:cursor-not-allowed disabled:opacity-40">
              {isCompleted ? "Update my proof" : "Finish sprint →"}
            </button>
          )}
        </div>
      ...
      {isCompleted && sprint.outputs && (
        <>
          <div className={`mt-6 rounded-xl border p-4 text-sm leading-6 ${evidenceState === "approved" ? "border-mint/35 bg-mint/10 text-paper/78" : evidenceState === "rejected" || evidenceState === "missing" ? "border-coral/35 bg-coral/10 text-paper/78" : "border-gold/35 bg-gold/10 text-paper/78"}`}>
            {evidenceState === "approved" ? (
              <><span className="font-bold text-mint">Approved.</span> This is labeled practice evidence, not employment experience.</>
            ) : evidenceState === "rejected" ? (
              <><span className="font-bold text-coral">Rejected.</span> Update the work and submit a new version when ready.</>
            ) : evidenceState === "missing" ? (
              <><span className="font-bold text-coral">Evidence missing.</span> Submit again to recreate it.</>
            ) : (
              <><span className="font-bold text-gold">Sprint complete.</span> Review the practice evidence before Career Forge uses it.</>
            )}
          </div>
          ...
          <div className="trust-panel mt-6 p-5 sm:p-6">
            ...
            <label className="mt-3 block">
              <span className="flex flex-wrap items-center gap-2"><span className="text-lg font-bold text-paper">Résumé / project bullet</span><CopyButton getText={() => sprint.outputs?.resumeBullet ?? ""} /></span>
              <span className="mt-1 block text-xs text-paper/50">Place this under projects or independent practice, never under employment.</span>
              <textarea aria-label="Résumé / project bullet" value={sprint.outputs.resumeBullet} rows={4} onChange={(event) => patchOutputs(sprint, "resumeBullet", event.target.value)} className="trust-input mt-3 w-full border px-3 py-2.5 text-sm text-ink" />
            </label>
          </div>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            {evidenceState === "pending" ? (
              <Link href="/profile#evidence-review" className="lab-pill-button px-5 py-2.5 text-sm font-black">Review this evidence →</Link>
            ) : application ? (
              <Link href="/applications" className="lab-pill-button px-5 py-2.5 text-sm font-black">Back to application →</Link>
            ) : (
              <Link href="/tailor" className="lab-pill-button px-5 py-2.5 text-sm font-black">Analyze another job →</Link>
            )}
      ...
      {!isCompleted && <Link href="/tailor" className="mt-6 inline-block text-sm font-bold text-paper/55 hover:text-cyan">← Back to job analysis</Link>}
    </section>
  );
}
