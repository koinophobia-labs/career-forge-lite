  ...
  return (
    <section className="trust-panel mt-8 overflow-hidden p-5 sm:p-6" aria-labelledby="pack-ready-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="trust-kicker text-xs font-bold uppercase">Generation complete</p><h2 id="pack-ready-title" className="mt-2 text-3xl font-bold text-paper">Your Résumé Pack is ready.</h2><p className="mt-2 text-sm text-paper/65">{pack.lanePacks.length} active lane{pack.lanePacks.length === 1 ? "" : "s"} · {pack.variants.length} generated résumés · {pack.receipt.evidenceUsed.length} approved evidence items traced · {pack.receipt.gapsLeftUnclaimed.length + pack.receipt.unsupportedClaimsRefused.length} gaps or unsupported claims left unclaimed</p></div>
        <div className="flex flex-wrap gap-2"><Link href="/tailor" className="lab-pill-button inline-flex min-h-11 items-center px-5 py-2.5 text-sm font-black">Tailor a résumé to a real job →</Link><button type="button" onClick={exportBundle} disabled={working} className="min-h-11 rounded-md border border-gold/45 bg-gold/10 px-4 py-2 text-sm font-bold text-gold transition hover:bg-gold hover:text-ink disabled:opacity-50">{working ? "Building ZIP…" : "Export complete pack"}</button>{lastExportCount !== null && <p className="w-full text-xs font-bold text-mint">Bundle prepared with {lastExportCount} résumé files plus LinkedIn materials and a README.</p>}</div>
      </div>
      ...
      <details className="mt-5 rounded-xl border border-white/10 bg-white/5 p-4"><summary className="cursor-pointer text-sm font-bold text-paper">Open evidence receipt</summary><div className="mt-3 grid gap-2 text-sm text-paper/60 sm:grid-cols-2"><p>Direct and combined claims supported: {pack.receipt.evidenceUsed.length}</p><p>Evidence omitted: {pack.receipt.evidenceOmitted.length}</p><p>Keywords included: {pack.receipt.keywordsIncluded.join(", ") || "None without evidence"}</p><p>Unsupported claims refused: {pack.receipt.unsupportedClaimsRefused.length}</p><p>Claims transferred from related experience: {pack.receipt.transferredClaims.length}</p><p>Gaps and duration left unverified: {pack.receipt.gapsLeftUnclaimed.join(" · ") || "None"}</p></div></details>
    </section>
  );
}
