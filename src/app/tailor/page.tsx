...
export default function TailorPage() {
  ...
  useEffect(() => {
    ...
    setAnalysis(analyzeJobPost(savedPost, state.profile, state.lanes.find((lane) => lane.id === application.laneId) ?? effectiveLane, state.dossier));
  ...
  function handleJobPostChange(nextPost: string) {
    const isDifferentPosting = Boolean(jobPost.trim()) && firstPostingLine(jobPost) !== firstPostingLine(nextPost) && (analysis !== null || Math.abs(nextPost.length - jobPost.length) > 40);
    setJobPost(nextPost);
    setAnalysis(null);
    if (isDifferentPosting) {
    ...
    if (isDifferentPosting || !companyEdited) setCompany(inferred.company);
    if (isDifferentPosting || !roleTitleEdited) setRoleTitle(inferred.roleTitle);
  }
  ...
  function applicationRecord(id: string, status: "drafting" | "applied", current?: ApplicationRecord): ApplicationRecord {
    ...
    const questions = questionPrompts.trim()
      ? questionPrompts
          ...
          .filter(Boolean)
          .map((prompt, index) => draftApplicationQuestion(prompt, state.dossier, `${id}-question-${index + 1}`))
    ...
    return {
      ...
      analysisGaps: analysis ? analysis.requirements.filter((req) => req.status !== "covered").map((req) => req.requirement).slice(0, 6) : current?.analysisGaps ?? [],
  ...
  function saveAsApplication(status: "drafting" | "applied"): string {
    const applicationId = savedApplicationId ?? createId("app");
    let events: ReturnType<typeof activationEventsForTransition> = [];
    update((current) => {
      const existing = current.applications.find((item) => item.id === applicationId);
      const record = applicationRecord(applicationId, status, existing);
      const next = {
        ...current,
        applications: existing
          ? current.applications.map((item) => (item.id === applicationId ? record : item))
          : [...current.applications, record]
      };
  ...
  function startRoleSprint(req: RequirementMatch) {
    ...
    let sprintId = "";
    update((current) => {
      const existing = current.roleSprints.find((item) => item.requirement === req.requirement && item.applicationId === applicationId);
      if (existing) {
      ...
      <section className="mx-auto max-w-5xl px-5 py-10 sm:px-8">
        ...
        {!profileReady && hydrated && (
          <div className="mt-6 rounded-xl border border-gold/30 bg-gold/10 p-4 text-sm leading-6 text-paper/75">
            Results will be more useful after you add some work history. You can still analyze the posting now. <Link href="/profile" className="font-bold text-gold underline-offset-2 hover:underline">Add work history</Link>
          </div>
        )}
        ...
        <div className="trust-panel mt-7 p-5 sm:p-7">
          ...
          <details className="mt-5 border-t border-white/10 pt-4">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label><span className="text-sm font-bold text-paper">Company</span><input value={company} onChange={(event) => { setCompany(event.target.value); setCompanyEdited(true); }} placeholder="Optional" className="trust-input mt-2 w-full border px-3 py-2.5 text-sm text-ink" /></label>
              <label><span className="text-sm font-bold text-paper">Role title</span><input value={roleTitle} onChange={(event) => { setRoleTitle(event.target.value); setRoleTitleEdited(true); }} placeholder="Optional" className="trust-input mt-2 w-full border px-3 py-2.5 text-sm text-ink" /></label>
          ...
          <div className="mt-8 grid gap-6">
            ...
            <div className="trust-panel p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-2.5">
                {analysis.requirements.length ? analysis.requirements.map((req) => {
                  ...
                  const existingSprint = state.roleSprints.find((item) => item.requirement === req.requirement && item.applicationId === savedApplicationId);
                  return (
                    <div key={req.requirement} className="rounded-lg border border-white/12 bg-obsidian/40 p-3.5">
                      ...
                      {eligibility?.eligible && req !== eligibleRequirement && <button type="button" onClick={() => startRoleSprint(req)} className="mt-2.5 text-xs font-bold text-cyan hover:text-gold">{existingSprint ? "Resume sprint →" : "Build proof for this →"}</button>}
            ...
            <details className="rounded-xl border border-white/12 bg-white/5 p-4">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                ...
                ) : <><Link href="/applications" className="rounded-md border border-white/15 px-4 py-2 text-sm font-bold text-cyan">Open applications →</Link><button type="button" onClick={() => saveAsApplication("drafting")} className="rounded-md border border-white/15 px-4 py-2 text-sm font-bold text-paper/70">Save changes</button></>}
                {hasFeature("tailored_resume_export") ? <button type="button" onClick={startTailoredResume} disabled={!effectiveBaseline} className="rounded-md border border-cyan/40 px-4 py-2 text-sm font-bold text-cyan disabled:opacity-40">Build tailored résumé</button> : <LockedActionPill feature="tailored_resume_export" label="Build tailored résumé" />}
              </div>
            </details>
          </div>
        )}
