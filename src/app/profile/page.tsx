  ...
  return (
    <section id="review" className="trust-panel mt-6 scroll-mt-28 p-5 sm:p-6" aria-labelledby="import-review-title">
      ...
      {batch.retainSourceFilenames
        ? <label className="mt-3 flex min-h-11 items-start gap-2 text-xs leading-5 text-paper/55"><input type="checkbox" checked onChange={(event) => onRetainFilenames(event.target.checked)} className="mt-1"/><span>Retain source filenames in approved evidence metadata. Turn this off before saving to omit them from durable evidence; pending conflict review still shows its sources.</span></label>
        : <p className="mt-3 text-xs leading-5 text-paper/55">Source filenames remain visible only while this review is pending so conflicts can be traced. They will not enter approved dossier evidence unless filename retention is enabled; exact source excerpts stay attached.</p>}
      <div className="mt-5 grid gap-5">
        {groups.map(([group, title]) => {
          const items = proposals.filter((item) => item.group === group);
          if (!items.length) return null;
          return <section key={group} className="rounded-xl border border-white/12 bg-obsidian/35 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold text-paper">{title} <span className="text-paper/45">({items.length})</span></h3><p className="mt-1 text-xs text-paper/48">Approved items become reusable only after saving; identity, ambiguity, and conflicts always require an individual choice.</p></div><button type="button" onClick={() => onApproveGroup(group)} className="min-h-11 rounded border border-mint/50 px-3 py-1.5 text-xs font-bold text-mint">Approve clear items</button></div><div className="mt-3 grid gap-3">{items.map((item) => <article key={item.id} data-proposed-field={item.proposedField ?? "legacy"} className={`rounded-lg border p-3 ${item.status === "approved" ? "border-mint/40 bg-mint/5" : item.status === "rejected" ? "border-coral/40 bg-coral/5" : item.conflictGroup ? "border-gold/45 bg-gold/5" : "border-white/10"}`}><input aria-label={`Edit proposal ${item.label}`} value={item.detail} onChange={(event) => onChange(item.id, event.target.value)} className="trust-input w-full border px-3 py-2 text-sm text-ink"/><p className="mt-2 text-xs font-bold text-paper/60">Proposed field: {(item.proposedField ?? item.group).replace("identity.", "")}</p><p className="mt-1 text-xs leading-5 text-cyan">Can support: {supportLabel(item.kind)}.</p>{item.conflictGroup && <p className="mt-2 rounded border border-gold/35 bg-gold/10 px-2 py-1.5 text-xs font-bold text-gold" role="status">Conflict: another source proposes a different value. Choosing this one rejects the other value; nothing is chosen automatically.</p>}{item.reviewRequired && !item.conflictGroup && <p className="mt-2 text-xs font-bold text-gold">Needs your individual confirmation before it can become durable.</p>}{item.classificationReasons?.length ? <ul className="mt-2 grid gap-1 text-xs text-paper/50">{item.classificationReasons.map((reason) => <li key={reason}>• {reason}</li>)}</ul> : null}{item.edited && <p className="mt-1 text-xs font-bold text-gold">Edited in this review; confirm the revised value explicitly.</p>}{(item.occurrenceCount ?? 1) > 1 && <p className="mt-1 text-xs font-bold text-cyan">Seen {(item.occurrenceCount ?? 1)} times; supporting sources were combined.</p>}{item.likelyDuplicateOf && <p className="mt-1 text-xs font-bold text-gold">Likely duplicate—compare before approving</p>}<details className="mt-2 text-xs text-paper/55"><summary className="cursor-pointer">Review source text and files</summary>{item.sourceExcerpts.map((source) => <p key={source} className="mt-2 border-l-2 border-cyan/30 pl-2">{source}</p>)}{item.sourceFilenames.length > 0 && <p className="mt-2">Sources: {item.sourceFilenames.join(", ")}</p>}</details><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => onDecision(item.id, "approved")} className="min-h-11 rounded bg-mint px-3 py-1.5 text-xs font-black text-ink">{item.conflictGroup ? "Choose this value" : "Approve"}</button><button type="button" onClick={() => onDecision(item.id, "rejected")} className="min-h-11 rounded border border-coral/50 px-3 py-1.5 text-xs font-bold text-coral">Reject</button><button type="button" onClick={() => onDecision(item.id, "proposed")} className="min-h-11 rounded border border-white/20 px-3 py-1.5 text-xs font-bold text-paper/65">Undecide</button><span className="ml-auto self-center text-xs uppercase text-paper/40">{item.confidence} confidence · {item.validation ?? "legacy"} · {item.status}</span></div></article>)}</div></section>;
        })}
      </div>
...
export default function DossierPage() {
  ...
  function decideProposal(id: string, status: ImportProposalRecord["status"]) {
    updateActiveBatch((batch) => {
      const selected = batch.proposals.find((item) => item.id === id);
      return {
        ...batch,
        updatedAt: new Date().toISOString(),
        proposals: batch.proposals.map((item) => {
          if (item.id === id) return { ...item, status };
          if (status === "approved" && selected?.conflictGroup && item.conflictGroup === selected.conflictGroup) return { ...item, status: "rejected" as const };
          return item;
        })
      };
    });
  ...
  function approveGroup(group: ImportProposalGroup) {
    updateActiveBatch((batch) => ({ ...batch, updatedAt: new Date().toISOString(), proposals: batch.proposals.map((item) => item.group === group && item.group !== "identity" && item.validation === "valid" && !item.reviewRequired && !item.conflictGroup ? { ...item, status: "approved" } : item) }));
  }
  ...
  function commitImportReview() {
    ...
    if (result.completed) {
      const readiness = assessDossierReadiness(result.state.dossier);
      setImportMessage(readiness.level === "not-ready"
        ? `Import decisions saved: ${result.approved} approved · ${result.rejected} rejected. Your career profile is still incomplete; confirm one structured role or project before downstream generation.`
        : `Import review complete: ${result.approved} approved · ${result.rejected} rejected. Only approved evidence can support outputs.`);
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
        ...
        {hydrated && (
          <>
            ...
            {activeBatch && <ImportReview batch={activeBatch} groups={importGroups} onChange={(id, detail) => updateActiveBatch((batch) => ({ ...batch, updatedAt: new Date().toISOString(), proposals: batch.proposals.map((proposal) => proposal.id === id ? { ...proposal, detail, candidateValue: detail, proposedField: "unresolved", roleCandidate: undefined, educationCandidate: undefined, projectCandidate: undefined, edited: true, status: "proposed", validation: "ambiguous", disposition: "unresolved", reviewRequired: true, classificationReasons: [...(proposal.classificationReasons ?? []), "User edited this candidate. The revised text stays unresolved rather than inheriting the parser's earlier field assignment; confirm it as evidence or use the structured profile editor below."] } : proposal) }))} onApproveGroup={approveGroup} onDecision={decideProposal} onCommit={commitImportReview} onMerge={mergeLikelyDuplicates} onDiscard={discardImportReview} onRetainFilenames={(retain) => updateActiveBatch((batch) => ({ ...batch, retainSourceFilenames: retain, updatedAt: new Date().toISOString() }))} />}
